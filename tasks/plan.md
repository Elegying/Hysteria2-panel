...
- [ ] 安装器和应用缺省协议改为 HTTP，HTTPS 保留为显式选项。
- [ ] 同一来源 IP 在观察窗口内连续错误 5 次后立即锁定 15 分钟。
- [ ] 锁定响应返回 `429` 和 `Retry-After`，登录错误保持通用文案。
- [ ] 消除不存在管理员账号时的明显密码校验时序差异。
- [ ] 更新管理员密码时撤销旧会话；所有面板响应禁止缓存并补齐安全头。
