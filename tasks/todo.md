...
- [ ] bootstrap 绑定的真实 Hysteria 灰度
  - Acceptance: 不创建真实用户；仅活动 grant 可认证；中央通过节点公网 IP 获得外部响应；失败不 ACK。
  - Verify: `python3 -m unittest tests.test_data_plane_bootstrap.AutomaticCanaryTests -v`
  - Dependencies: 持久完成器
...
- [ ] DNS 只读检测和自动准入
  ...
  - Verify: `python3 -m unittest tests.test_dns_admission -v`
  - Dependencies: 自动灰度
  - Files: `hysteria2_panel.py`, `install.sh`, `tests/test_dns_admission.py`, `tests/test_installer.py`
  - Scope: Medium
