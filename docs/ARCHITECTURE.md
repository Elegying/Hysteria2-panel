...
1. 管理员先手工配置 DNS 与云安全组，再生成绑定节点公网 IP 的一次性对接码；
2. 安装器检查并补齐依赖，生成 Ed25519 身份；面板核验短时对接码与来源 IP 后自动登记和确认身份；
...
6. 两个入口均通过后才确认安装；
7. 持久完成器清除待完成标记，面板显示对接结果。

当前一键流程不要求人工核对指纹，也不查询、等待或修改 DNS。旧指纹确认和 DNS 准入接口仅为兼容保留，旧 DNS 观察 timer 在安装后停用。
