...
普通用户只读接口见 [客户端本人用量接口 v1](CLIENT_USAGE_API_V1.md)，使用原节点密码，仅查询本人，不授予下述管理或机器权限。

下述内容是 Hysteria2-panel 的**内部协议参考**，面向开发者和维护者，不是对第三方承诺的通用公网 API。管理操作应使用同源网页；数据节点应使用安装器生成的固定版本 Agent，不要手工拼接签名请求。
