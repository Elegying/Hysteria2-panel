...
第四阶段的成功标准是 `.210` 能以现有 `vpn.ssrvpn.vip`、现有用户认证材料和完全
...
6. 永远不改变 `vpn.ssrvpn.vip`、现有用户 URI、token 派生、HMAC 或证书指纹。
...
DNS 变更前从 `.201` 或另一独立主机把 `vpn.ssrvpn.vip` SNI/证书校验固定解析到
...
- 不改变 `vpn.ssrvpn.vip`、现有用户 URI/token 派生、HMAC、证书 DER 指纹或公钥。
...
7. `.210` 保持不在 `vpn.ssrvpn.vip` 权威 DNS 中，直到第五阶段另行批准。
