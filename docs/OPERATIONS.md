...
即使没有用户，节点仍需通过 HTTPS 上报在线快照、确认流量检查点并接收控制命令。
这些管理请求和 DNS 查询经过公网网卡，但不计入 Hysteria 用户用量。

控制请求复用同一中央面板地址的 TLS 会话票据，减少完整握手和证书链传输；每次
请求完成后关闭 TCP 连接，不占用中央面板的空闲 HTTP 工作线程。最多每 10 分钟
重新创建 TLS 上下文并执行完整证书验证，地址变化或传输失败会丢弃已缓存的会话。
不支持会话恢复的面板仍使用完整握手；失败的 POST 不会由传输层自动重发。

只有在线快照为空、没有用户流量或待执行命令，且本周期账本和快照均已确认时，
节点才将下次轮询间隔提高到 4–5 秒。检测到用户、流量或控制命令后恢复约 2 秒的
轮询；未确认的流量继续保存在 spool 中。旧面板的独立接口回退路径保持原频率。
认证请求仍即时发送，43 秒的新鲜度限制保持不变；空闲时远程命令的轮询等待
最多增加到 5 秒，网络与处理时间另计。

实际节省量取决于面板证书链、网络和用户活动。短时采样折算出的每日流量只用于
估算，应使用服务商相同账期和计费口径的记录验证长期收益。
...
v0.39.25 起，一次统计响应可以超过 1000 个账户，每个上报批次仍限制为 1000 个账户和 240 KiB。
短暂落盘失败时，控制进程保留已清空的响应，停止继续清空统计，使用原批次编号重试；容量不足时先上传已有持久批次。
先修复磁盘空间、权限或 I/O 故障，再等待失败计数恢复，避免此时主动重启控制服务。
尚未写入任何存储的响应只能保存在进程内存中，不能保证断电或进程被强制终止后的恢复。

### 节点对接在防火墙检查时反复回滚

查看 `hysteria2-panel-node-onboarding.service` 的日志。已注册、心跳在线并不代表数据面部署完成。
没有启用 UFW/firewalld 时，安装器允许保留标准 `inet f2b-table/f2b-chain` 中严格匹配
TCP `22`、`addr-set-sshd`、计数器和 `drop` 的 SSH 防爆破规则；不会修改规则或清除封禁。
如果部署入口本身使用 `22`，或存在额外、范围更大的入站规则，仍停止安装并保留原防火墙。
不要通过停用 Fail2Ban 或清空规则来绕过检查。旧版本安装器可能把这条 SSH 规则误判为
影响代理入口的自定义策略，需要升级包含兼容修复的安装器。
...
原始入库计数不翻倍，重复批次仍只结算一次。 从 v0.39.17 起，机器每日账本按批次的 UTC 采样日归属；跨周期迟到上报补到原采样日，人工基线前的迟到记录只补历史，不重复计入人工已用。旧版本已确认的批次缺少完整原始数据，不自动重写其历史归属。机器累计和预算来自独立每日机器账本，不受用户重置、删除或恢复影响。人工“当前已用”填服务商控制台的计费值，保存后只将新增实际用量翻倍加到这个基线上；改重置日会重新建立基线，进入新周期按新周期账本计费。
