...
流量耗尽或换机时不要直接关服务器。先在面板点击“开始摘流”，手工从 `PUBLIC_HOST` 的 A/AAAA 删除旧节点 IP，等在线设备归零后点击“检查并安全停用”。中央只在 DNS 已无旧 IP、在线快照和流量 ACK 新鲜且设备数为 0 时下发固定 STOP；Agent 保持运行，私钥和 spool 不删除。原机重新使用可点击恢复，再把 IP 加回 DNS；更换空白服务器应在新机走“全新节点对接”，验收并加入 DNS 后归档旧记录。“安全重绑定”只用于同一台已经部署的数据节点重新连接面板，不负责把旧机私钥隔空迁移到空白新机。
