...
恢复完成后再对接节点。全新节点执行 `--join-node`，已有数据节点执行面板生成的 `--rebind-node`：后者保留 Ed25519 私钥、公钥、Hysteria 证书/私钥、运行中的数据面和 durable traffic spool，只原子替换中央注册状态；失败或重启从持久事务清单回滚。管理员必须逐字核对原 Ed25519 指纹并启用协议；在此之前新认证安全失败，但已建立 Hysteria 会话不因重绑定被主动重启。

数据面部署码会对主入口和账号专属 UDP `443` 同时应用中央当前策略，并事务化要求至少 16 MiB UDP 收发缓冲、`fq` 与内核 `bbr`。内核或 sysctl 不满足时恢复部署前状态并报错，不会把节点标记为已安装。全新节点从恢复后的默认面板取得 `FULL`；已有节点的重绑定本身不改变其当前数据面，若旧节点不是 `FULL` 或存在配置漂移，须在重绑定和协议门禁完成后重新签发数据面部署码。
