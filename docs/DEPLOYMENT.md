...
Hysteria 节点使用自签名证书并把 SHA-256 指纹固定在已发放链接中；全新管理面板默认使用独立域名和 Let’s Encrypt HTTPS，历史安装则原样保留现有面板协议。两套 TLS 身份不要混为一谈。自动静默更换节点证书会让所有仍固定旧指纹的客户端断连，因此项目不执行无人值守自动续签或轮换。
