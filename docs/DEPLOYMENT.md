...
分机器统计来源、每台机器的手工已用流量基线和每月重置日都随 SQLite 数据库进入备份。面板本机来源 ID 由一键安装器写入 `panel.env`，升级时原样保留；迁移到新服务器时，安装器先生成新的本机来源 ID，恢复旧数据库后旧服务器记录作为历史保留，新服务器从新来源继续累计。升级前无法可靠反推机器的既有流量只迁移一次到“升级前历史（未归属）”，不得伪装成本机或任一数据节点流量；管理员可在面板中精确删除该未归属来源，用户总流量和已归属机器账本不受影响。
