...
移除节点前先手工调整 DNS，避免客户端继续连接即将卸载的服务器。在线节点使用“一键断连”：先结算流量，再卸载本项目的远端服务、身份、配置、状态与受管网络设置。服务器已经失联时使用“删除对接”，只撤销中央授权并保留历史账本，不代表远端已清理。“开始摘流”等旧生命周期接口仅为兼容保留，不再是当前页面的操作步骤。
