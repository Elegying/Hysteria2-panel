...
发布工作流只接受严格的 `vX.Y.Z` 标签，并且必须从该标签引用显式调度。它会验证标签提交等于当前 `origin/main`、源码与安装器版本一致，并把常规 CI（包括 `mobile-tests`）、标签类型哨兵与六平台安装矩阵都绑定到该标签引用和精确提交；CI 同时扫描当前树和本次新增历史，阻止高置信秘密与已退役基础设施标识进入后续提交。随后创建或复用**草稿** Release，生成确定性的 SPDX 源码清单，为安装器和 SBOM 分别生成无密钥 Sigstore bundle，重新下载草稿资产逐字节比较并验签。`gh release edit --draft=false` 是工作流最后一个命令，前一步失败时 Release 保持不可公开的草稿状态。
...
主机 SSH 防护、日志轮转及本地备份空间策略见[主机维护](HOST_MAINTENANCE.md)。v0.39.28 起自动升级备份采用最多 3 份、30 天及 1 GiB 预算，并优先保护最新两个恢复点和本次升级备份；这与下述一键还原的恢复目录、失败包保留策略分别管理。
...
恢复完成后再对接节点。管理员先手工配置用户域名 DNS 与云安全组，在“对接管理”填写节点名称和公网 IP，点击“一键对接”，再到空白目标服务器以 root 执行签名命令。安装器按需补装系统依赖，面板核验短时凭据与来源 IP 后自动登记 Ed25519 身份；随后持久完成器执行签名心跳、协议启用、双入口数据面、网络优化与真实 Hysteria 出口验收，不再要求人工核对指纹。

当前面板只在生成命令前提醒 DNS，不查询、不等待、也不修改 DNS；旧 DNS 观察 timer 已停用。云安全组仍须预先允许主 Hysteria UDP 端口、UDP `443` 及相应 TCP 探针；遗漏会让真实验收失败并阻止 ACK。身份和重试单元持久化后，即使 SSH 断开或前台等候超时，后台仍会继续收尾；此时应查看 onboarding 服务日志，不要重复粘贴对接码。

已有数据节点的 `--rebind-node` 属于保留的兼容恢复入口：它保留 Ed25519 私钥、公钥、Hysteria 证书/私钥、运行中的数据面和 durable traffic spool，只原子替换中央注册状态；失败或重启从持久事务清单回滚。在协议和 bootstrap 门禁完成前新认证安全失败，但已建立 Hysteria 会话不因重绑定被主动重启。它不负责把旧机私钥迁移到空白新机。

移除节点前先手工调整 DNS，避免客户端继续连接即将卸载的服务器。在线节点使用“一键断连”：先结算流量，再卸载本项目的远端服务、身份、配置、状态与受管网络设置。管理员可在在线、离线或正在断连时使用“删除对接”，立即撤销中央授权并保留历史账本，无需远端确认。该操作不代表远端已清理，也不保证已有连接立即断开。“开始摘流”等旧生命周期接口仅为兼容保留，不再是当前页面的操作步骤。
...
从 v0.39.17 起，节点维护入口共用同一把锁；重复对接或升级不会同时写身份。维护停止前先阻断新认证，等待在途认证预算，再清退连接并将最终流量持久化；失败回滚保留最新未确认批次与协议状态。Hysteria 的踢出标记只作用于同用户的下一次流量事件，空闲或多个会话可能不能及时退出。维护必须确认在线为空且连续无新增流量；未能收敛时明确失败并恢复原先活动的认证与控制服务，保留身份、spool 和事务，等待会话退出后重试。不能直接删除 spool、强停或跳过结算。

面板升级在覆盖程序和建立升级事务前，使用已验证的候选程序预检：连续确认本机在线为空后，再收取并结算最终流量。失败恢复原面板及先前活动的 timer，保留正在运行的 Hysteria。备份清单同时保存候选维护程序及依赖，重启恢复不会退回旧版宽松清退逻辑。用户备份还原若在清退阶段失败，会经现有恢复收尾流程恢复原身份和服务。
