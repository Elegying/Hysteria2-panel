...
class H2GlassCapture extends StatefulWidget {
  const H2GlassCapture({
    super.key,
    required this.child,
  ...
  final Widget child;
  @override
  State<H2GlassCapture> createState() => _H2GlassCaptureState();
}

class _H2GlassCaptureState extends State<H2GlassCapture>
    with WidgetsBindingObserver {
  final _capture = ValueNotifier<H2GlassFrame?>(null);
  GlobalKey? _key;
  ...
  void _scheduleCapture() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final image = repaint
          ? boundary.toImageSync(pixelRatio: dpr)
          : previous.image;
      _capturedRevision = boundary.revision;
      _capturedDpr = dpr;
      _capture.value = H2GlassFrame(image, origin);
      if (previous != null && !identical(previous.image, image)) {
        _retire(previous.image);
      }
    }, debugLabel: 'H2 glass capture');
  }
...
class H2GlassBackgroundSource extends StatelessWidget {
  const H2GlassBackgroundSource({super.key, required this.child});
  final Widget child;
  @override
...
class _CaptureBackground extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    _RenderCaptureBackground renderObject,
  ) {
...
class H2GlassFrame {
  const H2GlassFrame(this.image, this.origin);
  final ui.Image image;
  final Offset origin;
  static void requestOriginUpdate(BuildContext context) => context
      .getInheritedWidgetOfExactType<_CapturedGlass>()
      ?.onBackgroundPaint();
  static ValueListenable<H2GlassFrame?>? listenableOf(BuildContext context) =>
...
class _CapturedGlass extends InheritedWidget {
  ...
  final ValueNotifier<H2GlassFrame?> frames;
