...
- 从“对接节点”生成短时一次性部署代码；新服务器注册后须由管理员逐字核对 Ed25519 公钥 SHA-256 指纹，验证通过后才能启用每分钟一次的签名心跳。当前阶段只显示节点在线状态，不复制 Hysteria 证书、HMAC 或用户数据，不启动新的 VPN 入口，也不修改 `vpn.ssrvpn.vip`、用户链接或证书指纹；
