...
> 对接节点达到预算 95% 后等待管理员手动撤出 DNS；确认后至少 24 小时且持续空闲才自动停用，不强踢用户。面板自带节点超过预算仍持续服务。操作与边界见 [使用教程](docs/USER_GUIDE.md#对接节点的-95-退出保护)。
...
| 分机器统计 | 用户总流量按实际用量；每台服务器的统计和预算按上传、下载之和的两倍计费 |
| 一键断连 | 在线节点自动卸载全部对接功能；管理员随时可单方面删除中央对接关系 |
...
生产环境可以先验证固定 Release 的发布身份、文件完整性和 shell 语法，再授予 root 权限。下面示例固定到 `v0.39.20`；安装其他版本时，请同时修改 `version`：
...
从 v0.39.16 起，安装器会在面板安装/升级、节点数据面部署/升级时共用本地出站路由检测：确认有 IPv4 路由而没有检测到 IPv6 路由时，两入口使用 Hysteria `direct.mode: "4"`；双栈或检测不确定时保留 `auto`。v0.39.15 及更早版本不包含此处理。检测只查询内核路由，不发送探测流量，不替服务器配置地址或网关；有路由也不代表 IPv6 互联网一定可达。以后配置好 IPv6 后，应重新运行包含此改动的受管安装/升级流程以重新检测。

IPv4 模式能让双栈域名使用 IPv4，不能将客户端传来的 IPv6 字面地址转换成 IPv4。Telegram 等应用可能直接请求 IPv6 地址，单纯换 DNS、过滤 AAAA 或修改服务器 `gai.conf` 无法修复这类请求。IPv6 请求被拒绝也不应导致整个 Hysteria 连接关闭；如果多个 IPv4 流一同断开，需要继续排查客户端、QUIC 链路和协议错误。

| 方案 | 能解决什么 | 边界 |
|---|---|---|
| 服务商分配原生 IPv6 地址与路由 | 直接访问 IPv6 目标 | 需要服务商资源，并验证实际出站 |
| 经 IPv4 连接受信任的双栈中转或隧道 | 为无原生 IPv6 的机器提供 IPv6 出口 | 增加依赖、时延和出口管理；安装器不自动接入第三方服务 |
| 客户端让 Telegram 使用 IPv4 | 减少 IPv6 目标失败 | 需要客户端支持并做实机对照；不等同于只修改 DNS |
| 服务器 IPv4 出站模式 | 明确降级，避免尝试无路由的 IPv6 出站 | IPv6-only 目标仍不可达，不保证减少已存在的快速失败或断连 |

Hysteria 支持用 ACL 将 IPv6 目标交给 SOCKS5 出站，但中转必须具备可用 IPv6；HTTP 出站只支持 TCP。参见 [Hysteria 官方出站说明](https://v2.hysteria.network/docs/advanced/Full-Server-Config/#outbounds)。已有私网拒绝与端口限制应继续保留，不应为兼容处理放宽访问边界。

## 日常使用路径

- **管理用户**：创建用户 → 设置设备和流量限制 → 分享 URI 或二维码；
- **接入节点**：先手工完成 DNS → 点击“一键对接”生成代码 → 在目标服务器以 root 执行 → 自动完成注册、部署和验收；
- **移除节点**：在线时点击“一键断连”完成远端卸载；任何状态下均可点击“删除对接”，立即撤销中央授权，无需远端确认；
